...
def sub(left: Tensor, right: Tensor) -> Tensor:
    ...
    if right.requires_grad:
        ...
        def grad_fn_right(grad: Array) -> Array:
            return np.multiply(np.ones_like(data) * -1, grad)  # частная производная по правому -1
...
def sigmoid(inp: Tensor) -> Tensor:
    data = special.expit(inp.data)  # сразу сигмоида
...
class SGD:
    ...
    def step(self):
        self.parameters.data -= np.multiply(self.parameters.grad.data, self.lr)
